...
{
	pinMode(context, 0, intPin, INPUT);
	std::cout << "initialising\n";
	if(!mpr121.begin(1, 0x39)) {
		rt_printf("Error initialising MPR121\n");
		return false;
	}
	std::cout << "enabling Proximity\n";
	mpr121.enableProximity(true);
	std::cout << "set Proximity interrupr threshold\n";
	mpr121.setProximityInterruptThreshold(0, 175);
	//std::cout << "enable Proximity Interrupt\n";
	//mpr121.enableProximityInterrupt();
...
void render(BelaContext *context, void *userData)
{
	for(unsigned int n = 0; n < context->audioFrames; n++) {
		...
		if(++readCount >= readIntervalSamples) {
			readCount = 0;
			// std::cout << "send the auxiliary task\n";
...
{
#ifdef DEBUG_MPR121
		// rt_printf("%d ", mpr121.readProximity());
		// rt_printf("\n");
#endif	
